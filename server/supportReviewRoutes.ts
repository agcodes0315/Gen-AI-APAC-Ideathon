...
export const supportReviewRouter =
  Router();
  ...
  async (
    ...
    if (
      ...
      return res.status(
        ...
        error:
          'Rating must be 1–5 and review text is required.',
        code:
          'INVALID_REVIEW',
      });
    }
...
/**
 * Public landing-page feed:
 * only approved + explicitly public reviews.
 */
supportReviewRouter.get(
  '/api/reviews/public',
  async (
 ...
 * After a decision, the item disappears from this queue.
 */
supportReviewRouter.get(
  '/api/admin/reviews',
  requireRole(
...
supportReviewRouter.patch(
  ...
  async (
    ...
    if (
      !reviewStates.includes(
        moderationState as any
      ) ||
      moderationState ===
        'pending'
    ) {
      return res.status(
        ...
        error:
          'A moderation decision must be approved, hidden, or rejected.',
        code:
    ...
    if (
      ...
      return res.status(
        ...
        error:
          'The user did not consent to public display.',
        code:
